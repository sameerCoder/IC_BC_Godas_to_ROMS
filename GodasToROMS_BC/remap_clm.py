...
#       flooded.to_netcdf("flooded.nc")
    ...
    if ndim == 3:
        ...
        #comment by sakib print('before vertical remap 1', dst_varz[:,928,324])
        #coment by sakib print('before vertical remap 2', dst_varz[:,460,350])
        print('vertical interpolation from standard z level to sigma')
        dst_var = pyroms.remapping.z2roms(dst_varz[::-1,:,:], dst_grdz, \
                          dst_grd, Cpos=Cpos, spval=spval, flood=False)
        #comment by sakib print('after vertical remap 1', dst_var[:,928,324])
       #comment by sakib  print('after vertical remap 2', dst_var[:,460,350])
    ...
    print("nc file:",nc)
    #sakib nc.variables['ocean_time'][0] = time
    nc.variables['time'][0]=time
